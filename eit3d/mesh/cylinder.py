"""
Cylinder mesh generation and caching with Gmsh.
Generated once, reused from disk on subsequent runs.
"""
...
from eit3d.config import (
    MESH_FILE, TOP_TAG, BOTTOM_TAG, LATERAL_TAG, VOLUME_TAG,
    MeshConfig,
)
...
class CylinderMesh:
    ...
    First run: generates mesh (~90s) and saves to cache/mesh.xdmf.
    ...
    def _generate(self) -> None:
        logger.info("Generating mesh (first time — will be saved to cache)...")
        gmsh.finalize()
        gmsh.initialize()

        half_h = self._config.height / 2.0
        gmsh.model.occ.addCylinder(0, 0, -half_h, 0, 0, self._config.height, self._config.radius)
        ...
        gmsh.model.addPhysicalGroup(3, [v[1] for v in gmsh.model.getEntities(dim=3)], tag=VOLUME_TAG)
        ...
        gmsh.finalize()
        logger.info("Mesh generated: %d cells, %d vertices", self.n_cells, self.n_vertices)
    ...
    def _load(self) -> None:
        logger.info("Loading mesh from disk (%s) ...", self._mesh_file)
        ...
        logger.info("Mesh loaded: %d cells, %d vertices", self.n_cells, self.n_vertices)
